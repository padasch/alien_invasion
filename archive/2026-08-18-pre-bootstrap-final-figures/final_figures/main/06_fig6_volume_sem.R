...
validate_phenology_overall_timing_sem <- function(file) {
  if (is.na(file) || !file.exists(file)) return(FALSE)

  required_cols <- c(
    "species", "effect", "component", "estimate", "p_boot", "n_boot_success"
  )
  effects <- tryCatch(
    readr::read_csv(file, show_col_types = FALSE),
    error = function(e) NULL
  )
  if (is.null(effects) || !all(required_cols %in% names(effects))) return(FALSE)

  totals <- effects %>%
    dplyr::filter(
      .data$component == "Path-summed total",
      .data$species %in% c("fagus", "quercus"),
      .data$effect %in% c("precipitation", "robinia", "culture")
    )
  if (nrow(totals) != 6L || any(totals$n_boot_success < 1000L)) return(FALSE)

  bundle_file <- file.path(dirname(file), "phenology-overall-timing-analysis.rds")
  bundle <- tryCatch(readRDS(bundle_file), error = function(e) NULL)
  if (is.null(bundle)) return(FALSE)
  settings <- bundle$settings

  timing_ok <- all(vapply(bundle$timing_index_data, function(x) {
    all(c("timing_z", "phenology_oriented_z", "n_transitions") %in% names(x)) &&
      all(x$n_transitions >= 2L) &&
      isTRUE(all.equal(x$phenology_oriented_z, -x$timing_z, tolerance = 1e-12))
  }, logical(1)))
  block_adjusted <- all(vapply(bundle$sem_fits, function(x) {
    mediator_terms <- attr(stats::terms(x$mediator), "term.labels")
    outcome_terms <- attr(stats::terms(x$outcome), "term.labels")
    "block" %in% mediator_terms && "block" %in% outcome_terms
  }, logical(1)))

  identical(as.integer(settings$stages), 2:4) &&
    identical(as.character(settings$spring_swc_start), "2025-03-04") &&
    identical(as.character(settings$spring_swc_end), "2025-04-02") &&
    isTRUE(settings$bootstrap_replicates >= 1000L) &&
    identical(settings$soil_type, "both") &&
    identical(settings$include_soil_treatment, FALSE) &&
    timing_ok &&
    block_adjusted
}

find_phenology_overall_timing_sem <- function() {
  root <- file.path(ALINV_PROJECT_ROOT, "output")
  files <- list.files(
    root,
    pattern = "sem-effect-decomposition[.]csv$",
    recursive = TRUE,
    full.names = TRUE
  )
  files <- files[grepl("phenology-overall-timing-analysis", files, fixed = TRUE)]
  if (!length(files)) return(NA_character_)

  date_part <- stringr::str_extract(files, "[0-9]{4}-[0-9]{2}-[0-9]{2}")
  files <- files[order(as.Date(date_part), file.info(files)$mtime, decreasing = TRUE, na.last = TRUE)]
  valid <- vapply(files, validate_phenology_overall_timing_sem, logical(1))
  if (!any(valid)) return(NA_character_)
  normalizePath(files[which(valid)[[1]]], winslash = "/", mustWork = TRUE)
}

ensure_phenology_overall_timing_sem <- function() {
  file <- find_phenology_overall_timing_sem()
  if (!is.na(file)) return(file)

  analysis_script <- file.path(
    ALINV_PROJECT_ROOT,
    "scripts",
    "12-phenology-overall-timing-analysis.R"
  )
  if (!file.exists(analysis_script)) {
    stop("Missing phenology overall-timing analysis script: ", analysis_script, call. = FALSE)
  }

  status <- system2(
    file.path(R.home("bin"), "Rscript"),
    c("--vanilla", shQuote(analysis_script), "--bootstrap=1000", "--cores=4")
  )
  if (!identical(status, 0L)) {
    stop("Phenology overall-timing analysis failed with exit status ", status, ".", call. = FALSE)
  }

  file <- find_phenology_overall_timing_sem()
  if (is.na(file)) {
    stop("Phenology overall-timing analysis did not create a valid SEM result bundle.", call. = FALSE)
  }
  file
}

read_phenology_transition_sem_totals <- function() {
  file <- ensure_phenology_overall_timing_sem()
  readr::read_csv(file, show_col_types = FALSE) %>%
    dplyr::filter(
      .data$component == "Path-summed total",
      .data$effect %in% c("precipitation", "robinia", "culture")
