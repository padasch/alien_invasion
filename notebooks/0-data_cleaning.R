...
df <- read_excel(fp, sheet = "Fluoropen QY") %>%
  ...
  mutate(
    tree_id = paste(boxlabel, tree, sep = "-"),
...
df <- df %>%
  left_join(meta_tree %>% select(tree_id, id_number), by = "tree_id") %>%
  select(id_number, date, qy) %>%
  arrange(id_number, date)
...
df <- read_excel(fp, sheet = "Tree Condition") %>%
  ...
  mutate(
    id_number = as.integer(ID_number),
    condition = as.double(leafstate),
    comment   = as.character(comment)
  ) %>%
  select(id_number, date, condition, comment) %>%
  arrange(id_number, date)
...
df <- read_excel(fp, sheet = "Senescence") %>%
  ...
  select(id_number, date, percent_senesced, chl1, chl2, chlavg, comment) %>%
  arrange(id_number, date)
...
df <- read_excel(fp, sheet = "Growth_Measurements_D_H") %>%
  ...
  select(id_number, date, diameter, height) %>%
  arrange(id_number, date)
...
## Soil Respiration (sheet: "Soil Respiration") ----
...
# get_data() is the helper we built earlier to load data and join meta.
# Example:
