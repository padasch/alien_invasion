...
# Production temporal-LMM container-cluster bootstrap used by Figures 3 and 4.
# The manuscript formulas and original species-level response scaling are held
# fixed; only uncertainty is estimated by resampling containers.
...
OUTPUT_DIR <- file.path(PROJECT_ROOT, "data", "final", "bootstrap", "temporal_lmm")
WORK_DIR <- file.path(tempdir(), "alinv-temporal-bootstrap")
...
writeLines(report, file.path(WORK_DIR, "comparison.md"))

message("Temporal bootstrap analysis completed: ", OUTPUT_DIR)
